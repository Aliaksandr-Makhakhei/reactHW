...
type Friend = { _id:string, name: { first:string, last:string }, picture:string, index:number }

const getFullname = (user:Users):string | null => {
  if (!user.name) {
    return null;
  }
...
const hasFriendWithId = (user:Users, id:string):boolean => {
  return user.friends.findIndex(friend => friend._id === id) !== -1;
}

const countActiveUser = (users:Array<Users>):number => {
  return users.reduce((result, user) => { 
    return user.isActive ? result + 1 : result;
  }, 0);
}

const checkAge = ((users:Users):number => {
